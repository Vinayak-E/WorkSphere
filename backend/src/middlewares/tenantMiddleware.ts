...
export const tenantMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      ...
      if (!req.isTenantConnected || req.tenantConnection?.name !== decoded.tenantId) {
    ...
    return
      ...
      try {
        ...
        const newAccessToken = jwt.sign({ tenantId: refreshDecoded.tenantId }, jwtSecret, { expiresIn: "15m" });
        ...
        if (!req.isTenantConnected || req.tenantConnection?.name !== refreshDecoded.tenantId) {
