...
export class ProjectService {
  ...
  async getManagerProjects(
    ...
    const [projects, total] = await Promise.all([
      this.projectRepository.getProjectsByManager(
        connection,
        query,
        skip,
        limit
  ...
  async getProjectDetails(
    ...
    const departmentId =
      project.department._id || project.department.toString();
    const departmentEmployees =
      await this.employeeRepository.getEmployeesByDepartment(
        connection,
        departmentId
      );
    const tasks = await this.projectRepository.getTasksByProjectId(
      connection,
      projectId
    );
  ...
  async getEmployeeTasks(
    ...
    const [tasks, total] = await Promise.all([
      this.projectRepository.getTasksByEmployee(
        connection,
        employeeId,
        search,
        status,
      ...
      this.projectRepository.countTasksByEmployee(
        connection,
        employeeId,
        search,
        status
