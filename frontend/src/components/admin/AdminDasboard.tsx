...
const PaymentDashboard = () => {
  ...
  const [companyTotalPages, setCompanyTotalPages] = useState<number>(1);
  const [recentPaymentsPage, setRecentPaymentsPage] = useState<number>(1);
  const [recentPaymentsTotalPages, setRecentPaymentsTotalPages] =
    useState<number>(1);
    ...
    const fetchRevenueStats = async () => {
      try {
        ...
        const result = await api.get(
          `/admin/revenue-stats?page=${recentPaymentsPage}&limit=10`
        );
        if (result.data.success) {
          ...
          result.data.data.recentPayments.forEach((payment: Payment) => {
    ...
    if (selectedCompanyId) {
      ...
      const fetchCompanyPayments = async () => {
        try {
          ...
          const result = await api.get(
            `/admin/company/${selectedCompanyId}/payments?page=${companyCurrentPage}&limit=10`
          );
          if (result.data.success) {
  ...
  const calculateGrowth = () => {
    ...
    const lastMonth =
      revenueStats.monthlyRevenue[revenueStats.monthlyRevenue.length - 1]
        .revenue;
    const previousMonth =
      revenueStats.monthlyRevenue[revenueStats.monthlyRevenue.length - 2]
        .revenue;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-1 gap-6">
            ...
            revenueStats.recentPayments.length > 0 ? (
              <>
                ...
                <div className="mt-4 flex items-center justify-between">
                  <button
                    onClick={() =>
                      setRecentPaymentsPage(prev => Math.max(prev - 1, 1))
                    }
                  ...
                  <button
                    onClick={() =>
                      setRecentPaymentsPage(prev =>
                        Math.min(prev + 1, recentPaymentsTotalPages)
                      )
                    }
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          {selectedCompanyId ? (
                  ...
                  <button
                    onClick={() =>
                      setCompanyCurrentPage(prev =>
                        Math.min(prev + 1, companyTotalPages)
                      )
                    }
            ...
            <div className="flex justify-center items-center h-64">
              <p className="text-gray-500">
                Select a company to view payment history
              </p>
            </div>
          )}
        </div>
